...
public class Checker implements Visitor<Type, Type> {
	...
	public Type visit(IfExp e, Env<Type> env) {
		...
		if (!(condType instanceof BoolT)) {
			return new ErrorT("The condition should have boolean type, found " + condType.tostring() + " in "
					+ ts.visit(e, null));
		}
	...
	public Type visit(ListExp e, Env<Type> env) {
		...
		for (Exp elem : elems) {
			...
			if (!assignable(type, elemType)) {
				return new ErrorT("The " + index + " expression should have type " + type.tostring() + " found "
	...
	public Type visit(RefExp e, Env<Type> env) {
		...
		return new ErrorT("The Ref expression expects type " + type.tostring() + " found " + expType.tostring() + " in "
				+ ts.visit(e, null));
	}
	...
	public Type visit(DerefExp e, Env<Type> env) {
		...
		return new ErrorT("The dereference expression expects a reference type " + "found " + type.tostring() + " in "
				+ ts.visit(e, null));
	}
	...
	public Type visit(FreeExp e, Env<Type> env) {
		...
		return new ErrorT("The free expression expects a reference type " + "found " + type.tostring() + " in "
				+ ts.visit(e, null));
	}
